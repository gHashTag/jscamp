...
С появлением `ES2015 (ES6)` со встроенной поддержкой модулей в JavaScript каждый `файл` стал предсталять собой `отдельный модуль.` Чтобы сделать объекты, функции⚙️, классы или переменные доступными для внешнего мира, достаточно просто `экспортировать` их, а затем `импортировать,` где это необходимо, в другие файлы проекта.
