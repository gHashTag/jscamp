...
<<<<<<< HEAD
Первый способ `// комментарий` в этом случае весь текст 📜 в строке после двойного слэша будет комментарием. Когда интерпретатор видит два 2️⃣ слэша подряд `//`, он сразу понимает, что всё, что идёт далее до конца строки, можно смело проигнорировать. Пока не обращайте внимание на синтаксис📖 кода📟, с ним мы детально познакомимся чуть позже.
=======
...
Второй 2️⃣ способ - это `/* */`, такой способ более гибок, чем первый 1️⃣ . Как только интерпретатор видит 👁️‍🗨️ `/*`, он думает: "Ага, значит, пока я не увижу 👁️‍🗨️ `*/`, то могу ни на что здесь не обращать никакого внимания!"
