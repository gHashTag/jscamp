...
Часть шаблона можно заключить в скобки `(...)` создав **скобочную группу**. Квантификаторы используемые после скобок применяются ко всей группе, а так же совпавшую группу можно поместить в отдельный массив.
...
### search
`str.search(regExp)` возвращает позицию первого совпадения шаблона `regExp` в строке `str`, если в строке ничего не было найдено, то вернётся `-1`. У этого метода есть **ограничение**, он умеет возврощать только позицию **первого** совпадения.
```jsx live
function learnJavaScript() {
  let str = 'lime banana orange'
  ,regExp = /banana/
...
### exec
Метод `exec()` принимает параметром строку, а возвращает массив с резульататми. У этого методу присутствуют свойства:
