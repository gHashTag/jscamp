...
Начнёться загрузка и установка данных пакетов. После установки каждого пакета будет соответствующая надпись о том что он установлен успешно (successful).
...
:::caution Для установки требуется порядка 20 Гб дискового пространства. Можно установить на диск пакет на диск D: и потребуется установить переменные окружения пользователя.
:::
...
Для того что бы настроить переменные среды нужно открыть панель управления.
...
Можно проверить работу эмулятора нажав на зеленую кнопку в форме триугольника, должен появится интерфейс в виде сотового телефона.
...
Что бы создать проект на React Native нужно запустить PowerShell от имени администратора, затем в нем с помощью команды `cd` перейти в папку c Вашим проектом, например `cd D:\MyProject`
...
После завершения загрузки переходим в папку с созданым проектом и вводим команду запуска `Metro Bundler`:
...
Далее подключаем телефон с включенным режимом отладки по USB, либо запускаем Эмулятор. Открываем новое окно PowerShell. И ввести во втором окне PowerShell переходим в папку с Вашим проектом и запускаем команду для запуска проекта:
