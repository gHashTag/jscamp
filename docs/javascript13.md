...
**Символьный класс** - это специальное обозначение, которое соответствует любому символу из определённого набора. Символьный класс обозначается обратным слешем `\`, символ последующий после слеша будет считаться специальным символом. Специальные символы используются для создания более сложных шаблонов.
