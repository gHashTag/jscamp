...
Примеры системных функций вы уже видели – это alert(), prompt() и confirm(). Давайте создавать свои.
...
```
Если честно, то это обыкновенная процедура, выполняющая последовательность комманд. В данном случае внутри тела находится единственная команда alert(), вызывающая системное сообщение "Hello!".
...
myMessage();
```
Обратите внимание, она срабатывает независимо (просто вызывается, `не присваивая возвращаемые параметры` кому-либо).
:::tip Запомни!
...
alert(x);
```
Функция `showNumberFive()` вернет просто цифру 5 во внешнию переменную x, значение которой выведем через системную функцию alert().
...
```
После объявления функции и её параметров мы можем вызвать (запустить) с конкретным значением (например: 5). 
...
Если ещё нет, то вкратце она состоит в следующем: `аргумент` — это значение, которое мы «передали» функции в скобочках, и которое присвоилось `формальному параметру` этой функции. 

Или так: `параметр` — это название переменной, указанное в объявлении функции и ожидающее присваивания значения в момент вызова функции. Присваиваемое параметру значение и есть аргумент. Всё понятно? Отлично. А ещё вот что: множество программистов используют эти термины (аргумент и параметр) взаимозаменяемо.
:::
...
alert('Уф!');
```
Испугались? Обратите внимание, что встроенные функции действуют по тем же законам, что и те, которые мы с вами мастерили собственноручно (названия в верблюжьемРегистре, а за ними скобки с аргументом). Главное отличие в том, что эти функции вам не нужно объявлять, они описани в самой системе. 

Давайте для закрепления выведем 4 сообщения подряд:
```javascript
alert('А это необходимо?');
alert("Если б это был настоящий сайт?");
alert('Ну ладно, повеселились и будет.');
alert('Эй! Ну хватит уже!!');
```
Скажу честно: подавляющее большинство пользователей (и разработчиков) терпеть не могут функцию alert(). Это выплывающее сообщение их донельзя раздражает. Короче, можно очень позабавиться с этой нашей новообретённой суперспособностью.
...
Нажмите в `браузере Google Chrome` сочетание клавиш `Ctrl + Shift + I` и на вкладде `Console` наберите:
```javascript
...
Math.random()*100;
```
Если вам вдруг понадобится случайное число или ряд в этом диапазоне, то эта функция вам очень пригодится.
...
Для следующего этапа очень важно, чтобы всё, что мы пока написали, оставалось одним цельным блоком кода:
```javascript
function logRandom() {
    ...
    console.log(task + numberBig);
}

logRandom();   // Выбрано число от 0 до 100: 59
```
...
Оптимизируем нашу функцию, сразу в return (а так делают очень часто !) укажем результат вычислений: объеденив строку 'Выбрано число от 0 до 100: ' с вложенным расчетом:
...
logRandom();   // Выбери число от 0 до 100: 48
```
...
myMessage(); // Привет, Sumerian School !
```
...
alert( userName ); // Sumerian School перед вызовом функции

myMessage();

alert( userName ); // Школа программистов, значение внешней переменной было изменено функцией
```
...
console.log( sum1(7, 2) ); // 9
console.log( sum2(7, 2) ); // 9
```
...
```javascript
let messHello = () => alert("Hello!");

messHello();
```
Функции-стрелки могут быть использованы так же, как и Function Expression.

### Вариант 8. Безимянные функции
 Анонимные (безимянные) функции зачастую создаются и сразу же запускаются, так как находятся внутри колбэк функций. Еще одно важное отличие в том, что именованные функции объявляются, а анонимные создаются динамически. Анонимные функции могут выступать в качестве значения переменной.
...
Важное отличие анонимных функций в том, что они должны объявляться до их вызова. Можете вспомнить нашу первую статью в серии. Там сказано, что обычные функции можно объявлять и вызывать в любом месте. Браузер парсит код перед его выполнением, подхватывая именованные функции по пути. 
...
Главная причина – область видимости функции. Можете вспомнить из прошлой статьи (Стандартные функции), что переменная, объявленная внутри функции, может быть вызвана только внутри этой функции. В остальном коде данная переменная не видна. 
...
### Заключение
Поняв суть стрелочных и самовыполняющихся функций, вы сможете передавать функции в переменные и перестроить код по модулям. Функции такого рода можно использовать также и для создания и возврата других функций. Тут речь уже заходит о замыкании, о котором мы поговорим в следующей статье.
