...
Существует два 2️⃣ варианта синтаксиса📖 для создания🏗️ `пустого` массива:
...
В массиве могут храниться 📦 элементы любого типа - число, булевое значение (true/false), строки, объекты или целые функции⚙️:
...
`Очередь` – один из самых распространённых вариантов применения массива. В области компьютерных🖥️ наук так называется упорядоченная коллекция элементов, поддерживающая два 2️⃣ вида операций:
...
Псевдомассивы отличается от массива тем, что:

1. Имеют дополнительные нечисловые свойства и методы
2. Имеют бесконечную размерность
3. Содержат только последовательные значения элементов
