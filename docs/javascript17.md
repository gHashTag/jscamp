...
function learnJavaScript() {
  ...
  let name = ''
  function say(name) { return 'Hello, ' + name + ' !' }
  // userInput() - функция высшего порядка
  function userInput(fn) { // в качестве параметра функция, пока еще не известно какая
    // присвоим переменной name конкретное значение
    name = 'Alex' 
    // Вызовем заранее неизвесную функцию с переменной `name`
    return fn(name) 
  }

  let result = userInput(say) // say - функция callback (обратного вызова), становиться ясно какая функция передается в качестве параметра
