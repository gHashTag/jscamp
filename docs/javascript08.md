...
Условные выражения могут работать не только с булевыми значениями. В следующих примерах мы будем иметь дело с небулевыми переменными, то есть с такими, которые не точно `true` или `false` (то есть не точно "истинны" или "ложны"). Так что мы, в общем-то, спокойно можем использовать их в скобках, как и булевы значения.
