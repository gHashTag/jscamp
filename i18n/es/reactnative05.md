...
`transparent`
This is a shortcut for `rgba (0,0,0,0)`, the same as in [CSS3](https://www.w3.org/TR/css-color-3/#transparent).
...
In what notation React Native does not support `rgb()` and `rgba()`?

1. octal
2. hexadecimal
3. functional

Does React Native support `hsl()` and `hsla()`?
