...
Перед тем как мы перейдем к стадиям "Модельная разработки", посмотрим на тадиционный метод разработки приложений - «Разработка по фичам».

Метод, при котором ставиться задача на словах с ссылкой на [Zepllin](https://zeplin.io) и в лучшем случае прототипа в [Marvel App](https://marvelapp.com).
...
Еще один способ разработки - это разработка по моделям, он же метод «Модельная разработка». Основное его отличие от «Разработка по фичам» в том, что изначально мы ставим задачу в виде типизируемой модели(схемы) TypeScript и GraphQL, что позваляет разработчику использовать типизирование кода не по остаточному принципу, как это обычно бывает, а фундоментально. И так мы изначально закладываем в техническое задание типизируемую модель реализации базы данных, что дает нам возможность контролировать точность выполнения задачи на протяжении всего жизненого цикла выполнения задачи из бэклога в done.
...
UI Components - это набор инструментов пользовательского интерфейса с исходным кодом, который изолирует связь с экранами и навигацией, а также со слоем логики, внутри кроссптатформеного-фреймворка-компонентов пользовательского интерфейса. 

Создание мобильного приложения React Native начинается с создания UI Components в [Storybook](https://storybook.js.org), из которых будет строится приложение. Это наши строительные блоки, атомы, молекулы, организмы, из которых состоит вся визуальная часть приложения(screens). 
...
### Основные приемущества создания UI Components в [Storybook](https://storybook.js.org):
...
Скорость - 5-10 минут, так как сразу деплоиться схема с шага два и при этом код для создания запросов на сервер писать не надо, так как работает кодогенерация. Вам больше не нужно писать эти запросы к API, вам их нужно только вызвать. Это значит, что больше не нужно писать тысячи строк с кодом запроса к серверу, так как в AWS Amplify они также генерируются в вашем проекте автоматически на основании вашего стора, определеннго все той же GraphQL модели. И выполняется это одной командой `amplify push`.

Подробнее и о том как задеплоить схему читать [здесь](https://www.jscamp.app/ru/docs/amplify-04/)

Разработчиков следует разделить на верстальщиков(Junior), атрбоардщиков(Middle) и проектировщиков(Senior). Любая фича декомпозируется на три группы разработчков, где в самом начале проектировщики выдают техническое задание в виде TypeScript и GraphQL модели, затем создаются компонеты верстальшиками и собираются экраны атрбоардщиками. 
