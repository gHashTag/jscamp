...
Метод называется «Модельная разработка», потому что любая история декoмпозируется на три задачи, где в одной задаче реализуется TypeScript модель, во второй GraphQL модель, а в третей ее деплой на сервер:
￼

## Шаг 1 - UI-kits - Верстка - TypeScript модель компонента

￼![ModelDevs!](/img/mvp/01.jpg)

Создание мобильного приложения React Native начинается с создания UI-Kit в [Storybook](https://storybook.js.org). Создается UI-kit компонентов, из которых будет строится приложение. Это наши строительные блоки, из которых состоит приложение. Все компоненты верстаются группами на одном экране. Благодаря тому, что мы делаем из него [Storybook](https://storybook.js.org) наши компоненты легко переносятся на [React Native for Web](https://necolas.github.io/react-native-web/). Благодаря чему мы получаем UI-kit не только для мобильной разработки, но можем использовать его и на сайте, получая ускорение процесса разработки в два раза по верстке, так как нам не нужно верстать компоненты для сайта отдельно. 
...
## Шаг 2 - Логика - GraphQL модель экрана
